...
const MAPBOX_TOKEN = (typeof process !== 'undefined' ? process.env.VITE_MAPBOX_TOKEN : undefined) || (import.meta as any).env?.VITE_MAPBOX_TOKEN || localStorage.getItem('amg_mapbox_token') || "";
...
export default function App() {
  ...
  return (
    <div className="h-screen w-full bg-[#1D180C] text-white font-sans flex flex-col overflow-hidden selection:bg-amg-gold/30 relative">
      ...
      <header className="h-20 shrink-0 border-b border-[#6C6961] bg-[#000000]/90 backdrop-blur-2xl flex items-center px-8 z-40 relative">
        {/* Logo */}
      ...
      <div className="flex-1 flex overflow-hidden">
                  ...
                  <div className="relative cursor-pointer group z-20">
                    ...
                    <div className={`absolute top-12 left-1/2 -translate-x-1/2 whitespace-nowrap px-4 py-2 rounded bg-[#0a0a0a]/95 backdrop-blur-xl border transition-all duration-500 ${
                      isActive 
                        ...
                        : 'border-[#6C6961] text-white/60 opacity-0 -translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 shadow-lg'
                    }`}>
        ...
        <div className="w-[30%] h-full overflow-y-auto bg-[#000000] relative custom-scrollbar">
          
          {/* Sub-navigation for Packs (Segmented Control Style) */}
          {selectedNode.packs.length > 1 && (
            <div className="sticky top-0 z-40 bg-[#000000]/90 backdrop-blur-2xl border-b border-[#6C6961] p-4">
              <div className="flex p-1.5 bg-[#2F2A1F] border border-[#6C6961] rounded-lg relative">
                    ...
                    <div className="bg-[#1D180C] border border-[#6C6961] rounded-[2rem] p-8 relative overflow-hidden shadow-inner">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        ...
                        <div className="space-y-5">
                          ...
                          <p className="text-[15px] text-white/60 leading-relaxed font-light border-l-2 border-[#6C6961] pl-5">
...
function Badge({ icon, text }: { icon: ReactNode, text: string }) {
  return (
    <div className="flex items-center gap-2 bg-[#2F2A1F] border border-white/[0.08] px-4 py-2 rounded-full shrink-0 shadow-sm">
    ...
    <div className={`border transition-all duration-500 rounded-[2rem] overflow-hidden ${isOpen ? 'bg-[#1D180C] border-amg-gold/20' : 'bg-[#1D180C] border-[#6C6961] hover:border-white/20'}`}>
      ...
      >
        <div className="flex items-center gap-6">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-500 ${isOpen ? 'bg-amg-gold text-[#000000]' : 'bg-white/[0.02] border border-[#6C6961] text-amg-gold'}`}>
            {icon}
          </div>
