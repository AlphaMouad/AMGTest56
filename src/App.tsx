...
const ENV_MAPBOX_TOKEN = (import.meta as any).env?.VITE_MAPBOX_TOKEN || (typeof process !== 'undefined' ? (process as any).env?.VITE_MAPBOX_TOKEN : undefined) || "";
...
export default function App() {
  ...
  return (
    <div className="h-screen w-full bg-[#1D180C] text-[#FFFFFF] font-sans flex flex-col overflow-hidden selection:bg-amg-gold/30 relative">
      ...
      <header className="h-20 shrink-0 border-b border-[var(--color-amg-border)]/30 bg-[#1D180C]/90 backdrop-blur-2xl flex items-center px-8 z-40 relative">
        {/* Logo */}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[70%] h-full relative bg-[#1D180C] overflow-hidden flex flex-col">
          
          {!MAPBOX_TOKEN ? (
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center z-10 bg-[#1D180C] relative">
              ...
              <div className="relative z-10 w-full max-w-lg flex flex-col items-center">
                ...
                <p className="text-[var(--color-amg-text-muted)] text-[15px] leading-relaxed font-light mb-10 text-center">
                ...
                <div className="w-full bg-[#1D180C] border border-[var(--color-amg-border)]/50 rounded-2xl p-8 backdrop-blur-md shadow-2xl relative overflow-hidden group">
                  ...
                  <form onSubmit={handleTokenSubmit} className="flex flex-col gap-5">
                    <div className="relative">
                      <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
                        <Shield className="w-4 h-4 text-[var(--color-amg-text-muted)] group-focus-within:text-amg-gold transition-colors duration-300" />
                      </div>
                      <input
                        type="text"
                  ...
                  <div className="mt-6 pt-5 border-t border-white/5 flex flex-col gap-2 text-center">
                    ...
                    <p className="text-[var(--color-amg-text-muted)] text-[10px] font-light mt-1">Alternatively, configure the <code className="font-mono text-[var(--color-amg-text-muted)]">VITE_MAPBOX_TOKEN</code> environment variable.</p>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div className="relative cursor-pointer group z-20">
                    ...
                    <div className={`absolute top-12 left-1/2 -translate-x-1/2 whitespace-nowrap px-4 py-2 rounded bg-[#0a0a0a]/95 backdrop-blur-xl border transition-all duration-500 ${
                      isActive 
                        ...
                        : 'border-white/10 text-[var(--color-amg-text-muted)] opacity-0 -translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 shadow-lg'
                    }`}>
          ...
          <AnimatePresence mode="wait">
            <motion.div 
              ...
              className="absolute bottom-8 left-8 right-8 p-6 rounded-[2rem] bg-[#1D180C]/90 backdrop-blur-2xl border border-[var(--color-amg-border)]/50 shadow-2xl"
            >
        ...
        <div className="w-[30%] h-full overflow-y-auto bg-[#1D180C] border-l border-[var(--color-amg-border)]/30 relative custom-scrollbar">
          
          {/* Sub-navigation for Packs (Segmented Control Style) */}
          {selectedNode.packs.length > 1 && (
            <div className="sticky top-0 z-40 bg-[#1D180C]/90 backdrop-blur-2xl border-b border-[var(--color-amg-border)]/30 p-4">
              <div className="flex p-1.5 bg-[var(--color-amg-tabs)] border border-[var(--color-amg-border)]/50 rounded-lg relative">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <motion.div 
                  ...
                  className="bg-[#1D180C] border border-[var(--color-amg-border)]/50 rounded-[2rem] p-10 mb-12 relative overflow-hidden group"
                >
                ...
                <div className="flex flex-col items-center justify-center p-12 bg-[#1D180C] border border-[var(--color-amg-border)]/50 rounded-[2.5rem] mb-10 relative overflow-hidden">
                  ...
                  <span className="font-heading text-[9px] tracking-[0.3em] text-[var(--color-amg-text-muted)] uppercase mb-8 relative z-10">Contextual Data Matrix</span>
                  
                  <div className="flex p-1 bg-[var(--color-amg-tabs)] border border-[var(--color-amg-border)]/50 rounded-full relative w-full max-w-md z-10">
                    {['INSTITUTIONAL', 'RETAIL'].map((t) => (
                      <button
                        ...
                        className={`relative flex-1 py-3 font-heading rounded-full text-[10px] font-bold tracking-[0.25em] uppercase z-10 transition-all duration-500 ${
                          view === t ? 'text-[#000000]' : 'text-[var(--color-amg-text-muted)] hover:text-white'
                        }`}
                      >
                        {view === t && (
                          <motion.div
                ...
                <div className="space-y-4 mb-16">
                  ...
                  >
                    <div className="bg-[#1D180C] border border-[var(--color-amg-border)]/50 rounded-[2rem] p-8 relative overflow-hidden shadow-inner">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        ...
                        <div className="space-y-5">
                          <div className="flex items-center gap-3 text-[var(--color-amg-text-muted)]">
                          ...
                          <p className="text-[15px] text-[var(--color-amg-text-muted)] leading-relaxed font-light border-l-2 border-white/10 pl-5">
...
function Badge({ icon, text }: { icon: ReactNode, text: string }) {
  return (
    <div className="flex items-center gap-2 bg-[var(--color-amg-tabs)] border border-[var(--color-amg-border)]/50 px-4 py-2 rounded-full shrink-0 shadow-sm">
      <div className="text-amg-gold">{icon}</div>
      <span className="font-heading text-[9px] tracking-[0.2em] text-[var(--color-amg-text-muted)] uppercase">{text}</span>
    </div>
  );
}
...
function StatCard({ label, value, highlight = false, delay = 0 }: { label: string, value: string, highlight?: boolean, delay?: number }) {
  return (
    <motion.div 
      ...
      className={`relative p-8 rounded-[2rem] overflow-hidden group ${highlight ? 'bg-amg-gold/[0.08] border-amg-gold/40' : 'bg-[#1D180C] border-[var(--color-amg-border)]/50'} border transition-all duration-700 hover:border-white/20`}
    >
      ...
      <span className="font-heading block text-[10px] tracking-[0.4em] text-[var(--color-amg-text-muted)]/70 uppercase mb-4 font-bold transition-colors duration-500 group-hover:text-[var(--color-amg-text-muted)]">{label}</span>
...
function Accordion({ title, isOpen, onClick, children, icon }: { title: string, isOpen: boolean, onClick: () => void, children: ReactNode, icon?: ReactNode }) {
  return (
    <div className={`border transition-all duration-500 rounded-[2rem] overflow-hidden ${isOpen ? 'bg-[#1D180C] border-amg-gold/20' : 'bg-[#1D180C] border-[var(--color-amg-border)]/50 hover:border-white/20'}`}>
      <button 
        onClick={onClick}
      ...
      >
        ...
        <div className={`w-10 h-10 rounded-full border flex items-center justify-center transition-all duration-500 ${isOpen ? 'rotate-180 bg-amg-gold border-amg-gold text-[#000000]' : 'bg-transparent border-white/10 text-[var(--color-amg-text-muted)]'}`}>
